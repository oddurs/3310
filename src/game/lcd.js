...
export function createLCD() {
  ...
  // Nokia font text helpers — pixel-perfect rasterized glyphs
  function drawText7(text, x, y) {
    let cx = x
  ...
  function drawText(text, x, y) {
    for (let i = 0; i < text.length; i++) {
      drawChar(text[i], x + i * 8, y)
    }
  }

  function drawTextCentered(text, y) {
    const w = text.length * 8
    drawText(text, Math.floor((LCD_W - w) / 2), y)
  }
  ...
  function drawMenu(gameState) {
    ...
    const barH = NOKIA_FONT_SIZE + 4     // 24 (2px above text + 20px font + 2px below)
    const itemGap = 1
    ...
    const listTop = BORDER + NOKIA_FONT_SIZE + 6  // 32
    ...
    drawText7('Select', Math.floor((LCD_W - selectW) / 2), LCD_H - BORDER - NOKIA_FONT_SIZE)
  }
  ...
  function drawLeaderboard(gameState) {
    ...
    // Separator line
    const sepY = BORDER + NOKIA_FONT_SIZE + 4
    ctx.fillRect(contentX, sepY, contentW, 1)

    // Score entries
    const entryTop = sepY + 4
    const entryH = NOKIA_FONT_SIZE + 2
    for (let i = 0; i < 5; i++) {
    ...
    drawText7('Back', Math.floor((LCD_W - backW) / 2), LCD_H - BORDER - NOKIA_FONT_SIZE)
  }
